...
function openBuddy() { if (state.buddyOpen) return; state.buddyOpen = true; buddyWindow.hidden = false; document.querySelector('#spawnButton').disabled = true; document.querySelector('#spawnButton').textContent = 'Buddy is here'; buddyWindow.classList.remove('idle'); setBuddyMessage(document.querySelector('#greetingInput').value || 'Hey! Ready to make a little progress?'); if (!state.timerRunning) startTimer(); playBuddyAnimation('popUp', () => { buddyWindow.classList.add('idle'); clearInterval(state.messageInterval); state.messageInterval = setInterval(() => setBuddyMessage(document.querySelector('#idleInput').value || 'Still with you. Keep going.'), 12000); }); }
function closeBuddy() { buddyWindow.classList.remove('idle'); clearInterval(state.messageInterval); playBuddyAnimation('exit', () => { state.buddyOpen = false; buddyWindow.hidden = true; document.querySelector('#spawnButton').disabled = false; document.querySelector('#spawnButton').textContent = 'Spawn buddy'; }); setBuddyMessage(document.querySelector('#exitInput').value || 'Nice work. See you next time!'); }
